...
import { exchangeYouTubeCodeForToken } from "@/lib/youtube";
...
function YouTubeCallbackHandler() {
  ...
  useEffect(() => {
    ...
    exchangeYouTubeCodeForToken(code).then((success) => {
      if (success) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black">
      <div className="text-center space-y-4 animate-fade-in">
        <div className="h-5 w-5 border-2 border-neutral-700 border-t-red-500 rounded-full animate-spin mx-auto" />
        <p className="text-sm text-neutral-500">Connecting to YouTube Music…</p>
      </div>
    </div>
  );
}
